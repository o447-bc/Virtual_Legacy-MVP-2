...
const StartTheirLegacyFormFields: React.FC<StartTheirLegacyFormFieldsProps> = ({
  onSuccess,
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      await signupWithPersona(email, password, "setup_for_someone", "legacy_benefactor", firstName, lastName);
      onSuccess?.();
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <Button
        ...
        disabled={isLoading}
      >
        {isLoading ? "Creating account..." : "Start Their Legacy"}
      </Button>
    </form>
  );
};

export default StartTheirLegacyFormFields;
