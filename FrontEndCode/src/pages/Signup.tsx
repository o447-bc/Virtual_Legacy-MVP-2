...
const Signup = () => {
  ...
  const { signup, signupWithPersona, isLoading } = useAuth();
  const [searchParams] = useSearchParams();

  // On mount, read invite token and pre-fill email from URL params
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (validateForm()) {
      ...
      if (isInvited && inviteToken) {
        await signupWithPersona(normalizedEmail, password, "benefactor_invited", "legacy_benefactor", firstName, lastName, inviteToken);
      } else {
      ...
      <Card className="w-full max-w-md">
        <CardHeader className="space-y-1">
          ...
          <CardDescription className="text-center">
            {isInvited ? "You've been invited to SoulReel. Create your account to accept." : "Enter your information to get started"}
          </CardDescription>
          {isInvited && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 mt-2">
              <p className="text-sm text-blue-700">
                ✉️ You're signing up via an invitation. Your email is pre-filled and cannot be changed.
              </p>
            </div>
          )}
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input 
                ...
                readOnly={isInvited}
                className={isInvited ? "bg-gray-100 cursor-not-allowed text-gray-600" : ""}
              />
              {isInvited && <p className="text-xs text-gray-500">This email was set by your invitation and cannot be changed.</p>}
